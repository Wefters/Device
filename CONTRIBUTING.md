# Contributing to @wefterjs/device

Thank you for your interest in contributing to `@wefterjs/device`, the official Wefter plugin for device hardware specifications, vendor UUID, battery status, charging listeners, and system locale.
...
- `plugin.json`: Manifest defining the plugin name (`device`), methods, hooks, Android permissions, and iOS Info.plist entries.
- `src/index.ts`: TypeScript entry point exporting the typed `Device` wrapper.
...
Android uses Build and BatteryManager. iOS uses UIDevice and sysctl.
...
By contributing to `@wefterjs/device`, you agree that your contributions will be licensed under the [MIT License](LICENSE).
